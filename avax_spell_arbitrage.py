...
SNOWTRACE_API_KEY = "API-KEY-HERE"
...
SLIPPAGE = 0.1 * PERCENT  # tolerated slippage in swap price
...
def main():
    ...
    while True:
        ...
        # get quotes and execute SPELL -> sSPELL swaps only if have a balance of SPELL
        if spell["balance"]:
        ...
        # get quotes and execute sSPELL -> SPELL swaps only if have a balance of sSPELL
        if sspell["balance"]:
...
def get_approval(token, router, user):
    if DRY_RUN:
        # pretend have unlimited approval
        return 2**256 - 1
    try:
...
def get_token_balance(token, user):
    if DRY_RUN:
        # return a fake balance for testing
        return 10000 * 10**18
    try:
